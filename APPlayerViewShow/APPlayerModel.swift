...
class APPlayerModel: NSObject, PrintFormatProtocol {
    ...
    var playingStatusDidChange: ((AVPlayer.TimeControlStatus) -> Void)?
    ...
    private var playingStatus: AVPlayer.TimeControlStatus = .playing
    ...
    /// 播放状态观察者
    private var playingStatusObservation: NSKeyValueObservation?
    
    // MARK: - 计算属性
    var bridgePlayingStatus: AVPlayer.TimeControlStatus {
        playingStatus
    }
    ...
    init(resource: APVideoResource, volumeView: MPVolumeView) {
        ...
        playingStatusObservation = player.observe(\.timeControlStatus, options: .new) { [weak self] player, change in
            guard let self = self else {
                return
            }
            
            playingStatus = player.timeControlStatus
            
            delegate?.apPlayer(playerModel: self, playingStatus: player.timeControlStatus)
            playingStatusDidChange?(player.timeControlStatus)
        }
    }
    ...
    deinit {
        ...
        playingStatusObservation?.invalidate()
    }
